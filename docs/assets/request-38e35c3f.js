	...
	We = e => {
		...
		return (
			...
			((typeof FormData == 'function' && e instanceof FormData) ||
				(w(e.append) && ((t = H(e)) === 'formdata' || (t === 'object' && w(e.toString) && e.toString() === '[object FormData]'))))
		);
	},
...
function B(e, t, { allOwnKeys: n = !1 } = {}) {
	...
	let r, s;
	if ((typeof e != 'object' && (e = [e]), P(e))) for (r = 0, s = e.length; r < s; r++) t.call(null, e[r], r, e);
	else {
...
const Xe = (e, t, n, { allOwnKeys: r } = {}) => (
	...
	at = e => {
		Re(e, (t, n) => {
			if (w(e) && ['arguments', 'caller', 'callee'].indexOf(n) !== -1) return !1;
			const r = e[n];
			if (w(r)) {
	...
	oe = '0123456789',
	Oe = { DIGIT: oe, ALPHA: V, ALPHA_DIGIT: V + V.toUpperCase() + oe },
...
const pt = e => {
	...
	ht = O('AsyncFunction'),
	mt = e => e && (q(e) || w(e)) && w(e.then) && w(e.catch),
	a = {
		isArray: P,
		isArrayBuffer: ye,
		isBuffer: je,
...
function z(e, t, n) {
	...
	(t = t || new FormData()),
		(n = a.toFlatObject(n, { metaTokens: !0, dots: !1, indexes: !1 }, !1, function (h, A) {
	...
	function l(f, h, A) {
		...
		if (f && !A && typeof f == 'object') {
			...
			else if ((a.isArray(f) && Et(f)) || ((a.isFileList(f) || a.endsWith(h, '[]')) && (S = a.toArray(f))))
				return (
					(h = ge(h)),
					S.forEach(function (_, Ue) {
						!(a.isUndefined(_) || _ === null) && t.append(i === !0 ? ie([h], Ue, o) : i === null ? h : h + '[]', c(_));
					}),
					!1
				);
		}
	...
	function y(f, h) {
		if (!a.isUndefined(f)) {
			if (p.indexOf(f) !== -1) throw Error('Circular reference detected in ' + h.join('.'));
			p.push(f),
				a.forEach(f, function (S, N) {
					(!(a.isUndefined(S) || S === null) && s.call(t, S, a.isString(N) ? N.trim() : N, h, b)) === !0 && y(S, h ? h.concat(N) : [N]);
				}),
				p.pop();
		}
	}
...
const ce = St,
	...
	Rt = typeof URLSearchParams < 'u' ? URLSearchParams : ee,
	...
	function t(n, r, s, o) {
		...
		return (
			...
			d
				...
				: ((!s[i] || !a.isObject(s[i])) && (s[i] = []), t(n, r, s[i], o) && a.isArray(s[i]) && (s[i] = Pt(s[i])), !u)
		);
	}
	...
	transformRequest: [
		function (t, n) {
			const r = n.getContentType() || '',
				...
				o = a.isObject(t);
			if ((o && a.isHTMLForm(t) && (t = new FormData(t)), a.isFormData(t))) return s && s ? JSON.stringify(Ce(t)) : t;
			...
			let u;
			if (o) {
				if (r.indexOf('application/x-www-form-urlencoded') > -1) return Nt(t, this.formSerializer).toString();
...
const It = R.isStandardBrowserEnv
	? (function () {
			return {
...
function Be(e, t) {
	return e && !Ht(t) ? Mt(e, t) : t;
}
const qt = R.isStandardBrowserEnv
	? (function () {
...
const $t = typeof XMLHttpRequest < 'u',
	Vt =
		$t &&
		function (e) {
			return new Promise(function (n, r) {
				...
				a.isFormData(s) &&
					(R.isStandardBrowserEnv || R.isStandardBrowserWebWorkerEnv ? o.setContentType(!1) : o.setContentType('multipart/form-data;', !1));
				...
				function p() {
					if (!c) return;
					const y = T.from('getAllResponseHeaders' in c && c.getAllResponseHeaders()),
						h = {
				...
				if (
					...
					(c.ontimeout = function () {
						...
						e.timeoutErrorMessage && (f = e.timeoutErrorMessage), r(new m(f, h.clarifyTimeoutError ? m.ETIMEDOUT : m.ECONNABORTED, e, c)), (c = null);
					}),
					R.isStandardBrowserEnv)
				) {
					const y = (e.withCredentials || qt(l)) && e.xsrfCookieName && It.read(e.xsrfCookieName);
	...
	getAdapter: e => {
		...
		if (!r)
			throw r === !1
				...
				: new Error(a.hasOwnProp(k, n) ? `Adapter '${n}' is not available in the build` : `Unknown adapter '${n}'`);
...
function fe(e) {
	return (
		...
		Wt.getAdapter(e.adapter || te.adapter)(e).then(
			function (r) {
				return G(e), (r.data = K.call(e, e.transformResponse, r)), (r.headers = T.from(r.headers)), r;
			},
			function (r) {
				return (
					Fe(r) ||
						(G(e),
						r && r.response && ((r.response.data = K.call(e, e.transformResponse, r.response)), (r.response.headers = T.from(r.response.headers)))),
					Promise.reject(r)
				);
			},
		)
...
const Q = { assertOptions: Kt, validators: ne },
	g = Q.validators;
class I {
	constructor(t) {
	...
	request(t, n) {
		...
		r !== void 0 &&
			...
			s != null &&
				(a.isFunction(s) ? (n.paramsSerializer = { serialize: s }) : Q.assertOptions(s, { encode: g.function, serialize: g.function }, !0)),
		...
		if (!d) {
			...
			for (f.unshift.apply(f, u), f.push.apply(f, c), b = f.length, l = Promise.resolve(n); p < b; ) l = l.then(f[p++], f[p++]);
			return l;
		}
		b = u.length;
		let y = n;
		...
		for (p = 0, b = c.length; p < b; ) l = l.then(c[p++], c[p++]);
		return l;
	}
	getUri(t) {
		t = x(this.defaults, t);
...
a.forEach(['delete', 'get', 'head', 'options'], function (t) {
	I.prototype[t] = function (n, r) {
		return this.request(x(r || {}, { method: t, url: n, data: (r || {}).data }));
	};
});
a.forEach(['post', 'put', 'patch'], function (t) {
	function n(r) {
...
E.mergeConfig = x;
E.AxiosHeaders = T;
E.formToJSON = e => Ce(a.isHTMLForm(e) ? new FormData(e) : e);
E.HttpStatusCode = Qt;
E.default = E;
const Yt = E,
	he = e => {
...
Le.interceptors.request.use(
	e => (localStorage.getItem('token'), { ...e }),
	e => Promise.reject(e),
);
Le.interceptors.response.use(
	e => {
		const { data: t, config: n } = e,
			r = t.code || 200;
		return he(r) || e.data.msg || he('default'), r === 200 ? t : r === 401 ? (localStorage.setItem('token', ''), t) : Promise.reject(t);
	},
	e => {
		const { response: t } = e;
		t && (t.status, void 0);
